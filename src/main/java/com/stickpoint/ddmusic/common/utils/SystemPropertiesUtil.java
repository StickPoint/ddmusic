...
public class SystemPropertiesUtil extends Properties {
    ...
    public void loadProperties(){
        try {
            ...
            SystemCache.SYS_PROPERTIES.putAll(new YamlUtils(this.getProperty("applicationProperties")));
        } catch (IOException e) {
    ...
    public void flushCacheProperties(){
        SystemCache.SYS_PROPERTIES.clear();
    }
    ...
    public void updateSystemProperties(){
        try {
            this.load(this.getClass().getClassLoader().getResourceAsStream("sys.properties"));
            LOGGER.log(Level.INFO,"数据加载成功");
            YamlUtils applicationProperties = new YamlUtils(this.getProperty("applicationProperties"));
            // 数据加载完毕后，将系统配置保留在系统内存中
            if (Objects.nonNull(applicationProperties.get(InfoEnums.APP_ENV_VERSION.getInfoContent()))
                    &&Objects.nonNull(SystemCache.SYS_PROPERTIES.get(InfoEnums.APP_ENV_VERSION.getInfoContent()))) {
                // 检查系统环境版本
                String systemEnvironmentVersion = applicationProperties.get(InfoEnums.APP_ENV_VERSION.getInfoContent());
                // 检查当前环境版本
                String currentEnvironmentVersion = (String) SystemCache.SYS_PROPERTIES.get(InfoEnums.APP_ENV_VERSION.getInfoContent());
                // 如果不一致，直接刷新系统配置
                if (systemEnvironmentVersion.equals(currentEnvironmentVersion)){
                    SystemCache.SYS_PROPERTIES.putAll(applicationProperties);
